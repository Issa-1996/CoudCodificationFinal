...
function connexionBD($data_base = 'bd_25')
{
    $connexion = mysqli_connect('localhost', 'root', '', $data_base) or die('Serveur inaccessible. Merci de reessayer plus tard.');
    return $connexion;
}
$connexion = connexionBD();


function getToutesBases()
{
    $connexion = connexionBD(); // ta base principale
    $sql = "SELECT * FROM codif_periode ORDER BY id_peri ASC";
...
function verifierUtilisateursToutesBases($num_etu)
{
    ...
    for ($i = 0; $i < count($bases) - 1; $i++) {
        ...
        // Vérifie si la table codif_paiement existe
        ...
        $infos = info_1($num_etu, $conn);
        if (!$infos) {
            mysqli_close($conn);
            continue;
        }

        $delai_depart = getAllDelai_2("depart", $infos[5], $conn);
        $delai_fermeture = getAllDelai_2("fermeture", $infos[5], $conn);

        if (!$delai_depart || !$delai_fermeture) {
            mysqli_close($conn);
            continue;
        }
        // Dates
        $date_debut = dateFromat($delai_depart['data_limite']);
        $date_fermeture = dateFromat($delai_fermeture['data_limite']);

        // Calcul du nombre de mois total
        $debut = new DateTime($date_debut);
        $fin = new DateTime($date_fermeture);
        $interval = $debut->diff($fin);
        $nb_mois = ($interval->y * 12) + $interval->m;

        // Récupère les mois déjà payés
        $tableau_situation_paye = getAllSituation_2($num_etu, $conn);
        $libelle = []; // toujours réinitialiser

        $j = 0;
        while ($situation = mysqli_fetch_array($tableau_situation_paye)) {
            $libelle[$j] = $situation['libelle'];
            $j++;
        }
        ...
        $nbr_mois_impaye = $nb_mois - $nbr_mois_payer;

        if ($nbr_mois_impaye > 0) {
            return $array = [
                'num_etu' => $num_etu,
                'nom' => $infos[3],
                'prenoms' => $infos[4],
                'libelle_paye' => $libelle,
                'niveauFormation' => $infos[5],
                'date_debut' => $date_debut,
                'date_fermeture' => $date_fermeture,
                'nb_mois_total' => $nb_mois,
...
//VALIDATION QUOTAS PAR LE CHEF DE SERVICE DE L HEBERGEMENT
...
function getPaiementWithDateInterval_2($date_debut, $date_fin, $username, $libelle = "")
{
    ...
    if ($rowTotal = $resultTotal->fetch_assoc()) {
        $totalMontant = isset($rowTotal['montantTotal']) ? $rowTotal['montantTotal'] : (isset($rowTotal['countPayments']) ? $rowTotal['countPayments'] * 5000 : 0);
    }
...
function getAllDelai_2($nature, $faculte, $connexion)
{
    $requete =  "SELECT * FROM codif_delai where nature ='$nature' AND faculte ='$faculte'";
    $resultRequete = mysqli_query($connexion, $requete);
    return $resultRequete->fetch_assoc();
...
function info_1($login, $connexion)
{
    //Recherche des infos de l'etudiant
    $rr = "select * from codif_etudiant where num_etu='$login'";
    $ee = mysqli_query($connexion, $rr);
...
function getAllSituation_2($num_etu, $connexion)
{
    $requeteSelect = "SELECT * FROM `codif_paiement` JOIN `codif_validation` ON codif_validation.id_val = codif_paiement.id_val JOIN `codif_affectation`
	on codif_affectation.id_aff = codif_validation.id_aff JOIN `codif_etudiant` on codif_etudiant.id_etu = codif_affectation.id_etu JOIN `codif_lit` on codif_lit.id_lit = codif_affectation.id_lit WHERE codif_etudiant.num_etu='$num_etu' ORDER BY id_paie ASC";
    $resulteRequete = $connexion->query($requeteSelect);
